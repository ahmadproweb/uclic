...
function RelatedPortfolios({
  ...
  currentPortfolio: PortfolioPostProps["portfolio"];
}) {
  const { theme: currentTheme } = useTheme();
  const isDark = currentTheme === "dark";
  const [relatedPortfolios, setRelatedPortfolios] = useState<
    PortfolioPostProps["portfolio"][]
  >([]);
    ...
    const fetchRelatedPortfolios = async () => {
      try {
        ...
        const filteredPortfolios = portfolios
          .filter(
            (p: PortfolioPostProps["portfolio"]) => p.id !== currentPortfolio.id
          )
          .slice(0, 2);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          <div>
            ...
            <p
              className={cn("mb-4", isDark ? "text-white/80" : "text-black/80")}
            >
              Notre portfolio regorge de projets innovants et de solutions
...
export default function PortfolioPostClientSide({
  portfolio,
}: PortfolioPostProps) {
  const { theme: currentTheme } = useTheme();
  const isDark = currentTheme === "dark";
  ...
  useEffect(() => {
    ...
    iframes.forEach((iframe) => {
      if (!iframe.parentElement?.classList.contains("video-container")) {
        ...
        container.className =
          "video-container relative pt-[56.25%] my-10 rounded-xl overflow-hidden shadow-lg";
    ...
    tables.forEach((table) => {
      if (!table.parentElement?.classList.contains("table-container")) {
        const container = document.createElement("div");
        container.className =
          "table-container overflow-x-auto my-10 rounded-lg shadow-md";
  ...
  return (
    <>
        ...
        <div className="max-w-[900px] mx-auto px-4 md:px-6 relative z-10">
          ...
          <div className="flex justify-between items-center mb-10">
            ...
            >
              ...
              <span>/</span>
              <Link href="/cas-clients" className="hover:underline">
                Cas clients
              </Link>
            </nav>
      ...
      <StickyShareButtons
        ...
        url={`/cas-clients/${portfolio.slug}`}
      />
      <ScrollToTop />

...
import { estimateReadingTime, formatDate } from "@/services/wordpress";
    ...
    >
      <div className="relative w-full h-48 overflow-hidden">
        <img
          ...
          ).replace(/\.(jpg|jpeg|png|gif)$/, "-400x250.$1")}.webp`}
          alt={
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="flex items-center gap-2 text-sm text-black/70">
          ...
          {formatDate(post.date)} • {estimateReadingTime(post.content.rendered)}{" "}
          min de lecture
        </div>
      </div>
    </Link>
...
export default function AuthorPage({ author }: AuthorPageProps) {
  ...
  const featuredPost =
    author.posts && author.posts.length > 0 ? author.posts[0] : null;
  ...
  return (
    ...
    >
          ...
          <div className="relative w-full h-[40vh] md:h-[50vh] mb-16 rounded-3xl overflow-hidden shadow-xl">
            ...
            <div className="absolute inset-0 flex flex-col justify-end p-8 md:p-14">
              <div className="max-w-5xl mx-auto w-full">
                ...
                <h2
                  className="text-3xl md:text-5xl font-bold max-w-3xl mb-4 text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.7)]"
                ...
                <div className="text-white/80 flex flex-wrap items-center text-sm space-x-4 mt-4">
                  ...
                  <span>•</span>
                  <span>
                    {estimateReadingTime(featuredPost.content.rendered)} min de
                    lecture
                  </span>
                </div>
                <Link
      ...
      <ScrollToTop />
      <StickyShareButtons url={window.location.href} title={author.name} />
    </section>
  );
}

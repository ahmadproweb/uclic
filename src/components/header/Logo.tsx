...
export function Logo() {
  ...
  useEffect(() => {
    // Récupérer le domaine du cache au chargement
    const cachedDomain = localStorage.getItem('customLogoDomain');
    if (cachedDomain) {
      setCustomDomain(cachedDomain);
    }
    ...
    if (loveParam) {
      ...
      if (domain) {
        localStorage.setItem('customLogoDomain', domain);
        setCustomDomain(domain);
      }
    }
  }, [searchParams]);

  const logoUrl = customDomain
    ? `https://img.logo.dev/${customDomain}?token=pk_MW4QolF6TPewpNsB9eZvDA`
    : null;

          ...
          <url>
            ...
            ${imageUrl ? `
              <image:image>
                ...
                <image:title>${post.title.rendered.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')}</image:title>
              </image:image>
            ` : ''}
          </url>
        `;
        ...
        return `
          <url>
            ...
            ${imageUrl ? `
              <image:image>
                <image:loc>${imageUrl}</image:loc>
                <image:title>${levee.title.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')}</image:title>
              </image:image>
            ` : ''}
          </url>
        `;
...
export async function GET() {
  try {
    ...
    return new NextResponse('Error generating discover sitemap', { status: 500 });
  }
} 
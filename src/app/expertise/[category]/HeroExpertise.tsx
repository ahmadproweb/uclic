...
import type { ExpertiseCategoryFields } from "@/lib/wordpress";

// Composant HeroBackground identique à la page d'accueil (avec motif + halo fixe)
...
export default function HeroExpertise({ categoryName, expertiseFields }: HeroExpertiseProps) {
  const { theme: currentTheme } = useTheme();
  const isDark = currentTheme === "dark";

  const title = expertiseFields.h1 || categoryName;

...
async function generateNewsSitemap() {
  ...
  const recentLevees = levees.filter(
    (levee) => new Date(levee.date) > twoDaysAgo
  );

  console.log(
    `After filtering: ${recentPosts.length} recent posts and ${recentLevees.length} recent levees`
  );
